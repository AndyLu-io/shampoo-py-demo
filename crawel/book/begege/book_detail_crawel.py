...
def fetch_book_detail(url):
    try:
        ...
        book_name = driver.find_element(By.CSS_SELECTOR, "#maininfo #info h1").text.strip()
        book_author = driver.find_element(By.CSS_SELECTOR, "#maininfo #info p").text.strip()
        book_desc = driver.find_element(By.CSS_SELECTOR, "#intro p").text.strip()
        clean_book_author = book_author.replace("&nbsp;", "").replace("作", "").replace("者：", "").strip()
        # 定位 img 标签（按需要修改 selector）
        img_element = driver.find_element(By.CSS_SELECTOR, "#fmimg img.lazy")
